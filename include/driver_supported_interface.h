...
//last_pos成员的初始化值需为0，客户代码可以通过声明static或者利用下面的方式进行
//初始化：interface_t var = {0}，但比较推荐使用第一种方法，这样能更好地隐藏其内
//部的实现以及当内部实现细节改变时减少对客户代码破坏
typedef struct{
   const char interfaces_string[MAX_INTERFACE_STRING_LENGTH];
   int last_pos;
}interface_t;
